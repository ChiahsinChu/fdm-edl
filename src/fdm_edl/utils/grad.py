...
class GradLaplacian1D:
    """
    Numerical 1D gradient and Laplacian operator for sampled data ``y ≈ f(x)``.

    The operator computes the first derivative ``dy/dx`` ("gradient" in 1D) and the
    second derivative ``d²y/dx²`` (the 1D Laplacian) from samples on a grid ``x``.
    ...
    def __call__(self, x, y, *, h=None):
        ...
        """
        return self.grad_laplacian(x, y, h=h)

    def grad_laplacian(self, x, y, *, h=None):
        """
        Compute both gradient and Laplacian.
        ...
        """
        if self.uniform:
            return _uniform_grad_lap(
                x, y, self.boundary_points, self.interior_points, h
            )
        return _nonuniform_grad_lap(x, y, self.boundary_points, self.interior_points)

    def grad(self, x, y, *, h=None):
        """
        Compute gradient (1st derivative) only.
        ...
        grad : jax.numpy.ndarray
            First derivative approximation with shape ``(n,)``.
        """
        g, _ = self.grad_laplacian(x, y, h=h)
        return g
    ...
    def laplacian(self, x, y, *, h=None):
        ...
        lap : jax.numpy.ndarray
            Second derivative approximation with shape ``(n,)``.
        """
        _, lap = self.grad_laplacian(x, y, h=h)
        return lap


def _uniform_grad_lap(x, y, boundary_points: int, interior_points: int, h):
    """
    Uniform-grid backend: fixed-coefficient finite differences.
    """
    x = jnp.asarray(x)
    y = jnp.asarray(y)
    hh = jnp.mean(x[1:] - x[:-1]) if h is None else jnp.asarray(h)
    hh2 = hh * hh
...
def _nonuniform_grad_lap(x, y, boundary_points: int, interior_points: int):
    """
    Nonuniform-grid backend: local Fornberg weights.
    """
    x = jnp.asarray(x)
    y = jnp.asarray(y)
    n = x.shape[0]
    ...
    # endpoints
    grad = grad.at[0].set(onesided_eval(0, 1, True))
    lap = lap.at[0].set(onesided_eval(0, 2, True))
    grad = grad.at[-1].set(onesided_eval(n - 1, 1, False))
    lap = lap.at[-1].set(onesided_eval(n - 1, 2, False))
    ...
    ii = jnp.arange(r, n - r)
    grad = grad.at[ii].set(jax.vmap(lambda i: centered_eval(i, 1))(ii))
    lap = lap.at[ii].set(jax.vmap(lambda i: centered_eval(i, 2))(ii))

    # near-boundary points not covered by centered stencil (only when r=2, i.e., 5-pt interior)
    if kI == 5:
        grad = grad.at[1].set(onesided_eval(1, 1, True))
        lap = lap.at[1].set(onesided_eval(1, 2, True))
        grad = grad.at[-2].set(onesided_eval(n - 2, 1, False))
        lap = lap.at[-2].set(onesided_eval(n - 2, 2, False))
